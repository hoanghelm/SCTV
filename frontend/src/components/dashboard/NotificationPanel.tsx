...
const NotificationItem = styled.div<{ type: string }>`
  ...
  border-left: 4px solid ${props => 
    props.type === 'PersonDetected' ? theme.colors.success : theme.colors.error
  };
`
...
export const NotificationPanel: React.FC = () => {
  ...
  const { notifications, detectionEvents } = useSelector((state: RootState) => state.streaming)
          ...
          <NotificationList>
            {allNotifications.map((notification, index) => (
              <NotificationItem key={index} type={notification.type}>
                <NotificationHeader>
                  <NotificationTitle>
                    {notification.type === 'PersonDetected' ? 'Person Detected' : notification.type}
                  </NotificationTitle>
                  <NotificationTime>
                ...
                {notification.source === 'detection' && (notification as any).detections && (
                  <DetectionDetails>
                    {(notification as any).detections.length} detection(s) with average confidence{' '}
                    {Math.round(
                      (notification as any).detections.reduce((sum: number, det: any) => sum + det.confidence, 0) /
                      (notification as any).detections.length * 100
                    )}%
                  </DetectionDetails>
                )}
              </NotificationItem>
