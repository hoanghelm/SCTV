...
const TestResult = styled.div<{ success: boolean }>`
  ...
  background-color: ${(props) =>
    props.success ? `rgba(0, 186, 124, 0.1)` : `rgba(249, 24, 128, 0.1)`};
  color: ${(props) =>
...
export const AddCameraModal: React.FC<AddCameraModalProps> = ({
  ...
  const handleTestConnection = async () => {
    if (!formData.streamUrl.trim()) {
      setTestResult({
        success: false,
        message: "Please enter a stream URL first",
      });
      return;
    }
    ...
    setIsLoading(true);
    try {
      const success = await cameraService.testCameraConnection(
        formData.streamUrl,
      );
      setTestResult({
        success,
      ...
      <FormRow>
        ...
        <FormGroup>
          ...
          <Input
            ...
            onChange={(e) =>
              handleInputChange("frameRate", parseInt(e.target.value) || 30)
            }
            $fullWidth
          />
        </FormGroup>
