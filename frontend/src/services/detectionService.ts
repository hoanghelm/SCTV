...
class DetectionService {
    ...
    try {
      ...
      if (personDetections.length > 0) {
        ...
        this.stats.avgConfidence =
          personDetections.reduce(
            (sum: number, det: DetectionResult) => sum + det.score,
            0,
  ...
  startDetectionForVideo(
    ...
    const interval = setInterval(async () => {
      ...
      if (
        this.shouldSendDetectionEvent(cameraId, detections) &&
        onRealtimeEvent
      ) {
  ...
  private shouldSendDetectionEvent(
    cameraId: string,
