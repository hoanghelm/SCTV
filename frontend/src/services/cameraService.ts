...
export class CameraService {
      ...
      try {
        ...
        if (i === retries) {
          if (error instanceof TypeError && error.message.includes("fetch")) {
            throw new Error(
              `Network error: Unable to connect to ${url}. Please check if the server is running and accessible.`,
            );
          }
          throw error;
        }
  ...
  async getCameras(): Promise<Camera[]> {
    try {
      const response = await this.fetchWithRetry(
        `${API_BASE_URL}/api/v1/stream/cameras`,
      );
  ...
  async getCameraById(cameraId: string): Promise<Camera> {
    try {
      const response = await this.fetchWithRetry(
        `${API_BASE_URL}/api/v1/stream/camera/${cameraId}`,
      );
  ...
  async createTestStream(name: string = "Test Stream"): Promise<void> {
    try {
      const response = await this.fetchWithRetry(
        `${API_BASE_URL}/api/v1/stream/test/create-stream`,
        {
          method: "POST",
          body: JSON.stringify({
            name,
  ...
  async registerCamera(cameraData: {
    ...
    try {
      const response = await this.fetchWithRetry(
        `${API_BASE_URL}/api/v1/stream/camera/register`,
        {
          method: "POST",
          body: JSON.stringify({
  ...
  async testCameraConnection(streamUrl: string): Promise<boolean> {
    try {
      const response = await this.fetchWithRetry(
        `${API_BASE_URL}/api/v1/stream/camera/test`,
        {
          method: "POST",
          body: JSON.stringify({
            streamUrl,
